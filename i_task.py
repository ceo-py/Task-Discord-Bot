...
@client.tree.command(name="percent", description="Link with info about calculating percents!")
async def percent(interaction: discord.Interaction):
    ...
    embed.add_field(name="**Пример: 1**",
                    value='Имаме 10 ябълки. 10 ябълки са 100%. Изяждаме 2 ябълки. От тук следва, че сме изяли **__2 / 10 х 100% = 20%__** '
                          ...
                          ' или в нашия пример:' \
                          ' **__20%  = 2 ябълки х 100  / 10 ябълки__**', inline=False)
    embed.add_field(name="**Пример: 2**",
                    ...
                    inline=False)
    embed.add_field(name="**Намиране на число по даден процент**",
...
@client.tree.command(name="itask", description="It will show you how to use the bot for task solutions.")
